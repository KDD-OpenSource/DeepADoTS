...
class RecurrentEBM(object):
    ...
    def _build_model(self, n_visible):
        self.input_data, self.lr, self.batch_size = self._create_placeholders(n_visible)
        self.W, self.Wuh, self.Wux, self.Wxu, self.Wuu, self.bu, \
        ...
        self.BH_t = tf.tile(self.BH_t, [self.batch_size, 1])
        self.BX_t = tf.tile(self.BX_t, [self.batch_size, 1])
