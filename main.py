...
def get_detectors():
    if os.environ.get("CIRCLECI", False):
        tensorflow_detectors = [RecurrentEBM(num_epochs=2), Donut(num_epochs=5)]
        pytorch_detectors = [LSTMAD(num_epochs=5), DAGMM(num_epochs=2),
                             LSTMED(num_epochs=2), DAGMM(num_epochs=2, autoencoder_type=LSTMAutoEncoder)]
        return tensorflow_detectors, pytorch_detectors
    else:
        tensorflow_detectors = [RecurrentEBM(num_epochs=15), Donut()]
        pytorch_detectors = [LSTMAD(), LSTMED(num_epochs=40),
...
def run_pipeline():
    ...
    tensorflow_detectors, pytorch_detectors = get_detectors()

    for detectors in [pytorch_detectors, tensorflow_detectors]:
