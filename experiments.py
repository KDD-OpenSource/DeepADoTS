...
def run_multivariate_experiment(seeds, runs, output_dir=None, store_results=True):
    return run_experiment_evaluation(seeds, runs, output_dir, "multivariate", store_results=store_results)
    ...
    for seed in seeds:
        if anomaly_type == "extreme":
            ...
            yield [MultivariateAnomalyFunction.get_multivariate_dataset(
                        mv_outlier_type, random_seed=seed, features=dim, group_size=gsize or dim,
