...
def run_pollution_experiment(detectors, outlier_type='extreme_1', output_dir=None, steps=5, seed=42):
    datasets = [
        SyntheticDataGenerator.get(f'{outlier_type}_polluted', seed, pollution) for pollution in np.linspace(0, 0.5, steps)
    ]
...
# nan values.
def run_missing_experiment(detectors, outlier_type='extreme_1', output_dir=None, steps=5, seed=42):
    datasets = [
        SyntheticDataGenerator.get(f'{outlier_type}_missing', seed, missing) for missing in np.linspace(0, 0.9, steps)
    ]
    evaluator = Evaluator(datasets, detectors, output_dir)
    evaluator.evaluate(seed)
    evaluator.benchmark_results = evaluator.benchmarks()
...
def run_extremes_experiment(detectors, outlier_type='extreme_1', output_dir=None, steps=10, seed=42):
    datasets = [
        SyntheticDataGenerator.get(f'{outlier_type}_extremeness', seed, extreme) for extreme in np.linspace(1, 10, steps)
    ]
...
def run_multivariate_experiment(detectors, output_dir=None, seed=42):
